<?php
// Create fresh database with different name to avoid tablespace issues
...
try {
    ...
    $conn->exec("DROP DATABASE IF EXISTS handtoglobal");
    echo "✓ Dropped old database 'handtoglobal'\n";
    
    // Create fresh database
    $conn->exec("CREATE DATABASE handtoglobal CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
    echo "✓ Created fresh database 'handtoglobal'\n";
    
    // Switch to the database
    $conn->exec("USE handtoglobal");
    ...
    // Update config.php to use the new database name
    $configFile = __DIR__ . '/config.php';
    if (file_exists($configFile)) {
        $configContent = file_get_contents($configFile);
        $configContent = str_replace("define('DB_NAME', 'globalhand');", "define('DB_NAME', 'handtoglobal');", $configContent);
        file_put_contents($configFile, $configContent);
        echo "✓ Updated config.php with new database name\n";
    }
