<?php
/**
 * Comprehensive Database Repair Script for HandToGlobal
 * 
 * This script fixes ALL database schema issues across the entire project
 * Run this script once to repair all database structure issues
 */
...
echo "=== HANDTOGLOBAL COMPREHENSIVE DATABASE REPAIR ===\n\n";
...
function tableExists($conn, $tableName) {
    try {
        ...
        echo "❌ Failed to check table $tableName: " . $e->getMessage() . "\n";
        return false;
    }
}
...
foreach ($employeesColumns as $column => $definition) {
    safeAddColumn($conn, 'employees', $column, $definition);
}
...
foreach ($levelsColumns as $column => $definition) {
    safeAddColumn($conn, 'levels', $column, $definition);
}
...
try {
    ...
    $requiredTables = [
        'users', 'admins', 'employees', 'levels', 'tasks', 'combos', 
        'invitation_codes', 'testimonials', 'contacts', 'deposits', 'withdrawals',
        'completed_tasks', 'user_levels', 'balance_logs', 'user_limits', 
    ...
    echo "❌ Tables missing: $errorCount\n";
    
    if ($errorCount === 0) {
        echo "✅ ALL REQUIRED TABLES CREATED/VERIFIED SUCCESSFULLY!\n";
        echo "✅ All missing columns added safely!\n";
        echo "✅ Database schema is now complete and ready for use!\n";
        echo "\nNext steps:\n";
        echo "1. Test admin panel functions\n";
        echo "2. Test user registration and login\n";
        echo "3. Test deposit and withdrawal functionality\n";
        echo "4. Test task completion and level progression\n";
        echo "\nYour HandToGlobal database is now fully repaired!\n";
    } else {
        echo "❌ SOME TABLES FAILED TO CREATE\n";
