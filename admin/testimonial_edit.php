    ...
    if (empty($name)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$type, $name, $content, $display_order, $is_active, $testimonial_id]);
            
            $msg = "Testimonial updated successfully!";
            
            // Redirect to testimonials list
            if (empty($error)) {
                redirect('testimonials.php?msg=' . urlencode('Testimonial updated successfully!'));
            }
...
<head>
    ...
    <title>Edit Testimonial - <?php echo htmlspecialchars(get_site_name()); ?> Admin</title>
...
<!-- MANAGEMENT Section -->
        ...
        <div class="main-content">
            <?php admin_back_button('testimonials.php'); ?>
            <?php if ($msg): ?>
            ...
            <div class="breadcrumb">
                <a href="testimonials.php">Testimonials</a> > Edit
            </div>
            ...
            <div class="form-card">
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control-select" required>
                            <?php foreach ($types as $type): ?>
                                <option value="<?php echo $type; ?>" <?php echo (($testimonial['type'] ?? ($_POST['type'] ?? 'Client')) == $type) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" class="form-control" value="<?php echo htmlspecialchars($testimonial['name'] ?? ($_POST['name'] ?? '')); ?>" required>
                    </div>
                    
                    <!-- Content -->
                    <div class="form-group">
                        <label class="form-label">Content</label>
                        <textarea name="content" class="form-control-textarea" required><?php echo htmlspecialchars($testimonial['content'] ?? ($_POST['content'] ?? '')); ?></textarea>
                    </div>
                    
                    <!-- Order -->
                    <div class="form-group">
                        <label class="form-label">Order</label>
                        <input type="number" name="display_order" class="form-control" value="<?php echo htmlspecialchars($testimonial['display_order'] ?? ($_POST['display_order'] ?? '0')); ?>">
                    </div>
                    
                    <!-- Active -->
                    <div class="form-group">
                        <div class="checkbox-group">
                            <input type="checkbox" name="is_active" class="checkbox-input" <?php echo (($testimonial['is_active'] ?? 0) || (isset($_POST['is_active']))) ? 'checked' : ''; ?>>
                    ...
                    <!-- Button -->
                    <div class="form-group">
                        <button type="submit" name="update_testimonial" class="btn btn-primary">Update</button>
                    </div>
                </form>
            </div>
            <?php endif; ?>
