...
try {
    ...
    $error = "Failed to fetch combo: " . $e->getMessage();
}
            ...
            if (!$stmt->fetch()) {
                ...
                if (count($tasks) !== 2) {
                    ...
                    $stmt = $conn->prepare("
                        ...
                        deposit_amount = ?, 
                        message = ?,
                        status = CASE WHEN ? = 1 AND status = 'Pending' THEN 'Active' ELSE status END,
                        updated_at = NOW()
                        WHERE id = ?
                    ");
                    $stmt->execute([$user_id, $level, $start_task_id, $end_task_id, $multiplier, $deposit_amount, $message, $activate_now, $combo_id]);
                    
                    $msg = "Combo updated successfully!";
                    
                    // Redirect to combos list
                    if (empty($error)) {
                        redirect('combos.php?msg=' . urlencode('Combo updated successfully!'));
                    }
...
$level_tasks = [];
$selected_level = $_POST['level'] ?? $combo['level'] ?? '';
...
<head>
    ...
    <title>Edit Combo - <?php echo htmlspecialchars(get_site_name()); ?> Admin</title>
...
<!-- MANAGEMENT Section -->
        ...
        <div class="main-content">
            ...
            <div class="breadcrumb">
                <a href="combos.php">Combos</a> > Edit
            </div>
                ...
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="user_id" class="form-control-select" required>
                            ...
                            <?php foreach ($users as $user): ?>
                                <option value="<?php echo $user['id']; ?>" <?php echo (($combo['user_id'] ?? ($_POST['user_id'] ?? '')) == $user['id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="level" class="form-control-select" id="levelSelect" required onchange="loadTasks()">
                            ...
                            <?php foreach ($levels as $level): ?>
                                <option value="<?php echo $level; ?>" <?php echo (($combo['level'] ?? ($_POST['level'] ?? '')) == $level) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="start_task_id" class="form-control-select" id="startTaskSelect" required>
                            ...
                            <?php foreach ($level_tasks as $task): ?>
                                <option value="<?php echo $task['id']; ?>" <?php echo (($combo['start_task_id'] ?? ($_POST['start_task_id'] ?? '')) == $task['id']) ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="end_task_id" class="form-control-select" id="endTaskSelect" required>
                            ...
                            <?php foreach ($level_tasks as $task): ?>
                                <option value="<?php echo $task['id']; ?>" <?php echo (($combo['end_task_id'] ?? ($_POST['end_task_id'] ?? '')) == $task['id']) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($task['title']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="multiplier" class="form-control" value="<?php echo htmlspecialchars($combo['multiplier'] ?? ($_POST['multiplier'] ?? '6')); ?>" step="0.1" min="1" required>
                    </div>
                    
                    <!-- DepositAmount -->
                    <div class="form-group">
                        <label class="form-label">DepositAmount</label>
                        <input type="number" name="deposit_amount" class="form-control" value="<?php echo htmlspecialchars($combo['deposit_amount'] ?? ($_POST['deposit_amount'] ?? '0')); ?>" step="0.01" min="0" required>
                    </div>
                    
                    <!-- Message -->
                    <div class="form-group">
                        <label class="form-label">Message</label>
                        <textarea name="message" class="form-control-textarea" placeholder="MessagePlaceholder"><?php echo htmlspecialchars($combo['message'] ?? ($_POST['message'] ?? '')); ?></textarea>
                    </div>
                    
                    <!-- ActivateNow -->
                    <div class="form-group">
                        <div class="checkbox-group">
                            <input type="checkbox" name="activate_now" class="checkbox-input" <?php echo (($combo['status'] === 'Active') || (isset($_POST['activate_now']))) ? 'checked' : ''; ?>>
                    ...
                    <!-- Update Button -->
                    <div class="form-group">
                        <button type="submit" name="update_combo" class="btn btn-primary">Update</button>
                    </div>
                </form>
            </div>
            <?php endif; ?>
    ...
    <script>
        function loadTasks() {
            ...
            if (level) {
                ...
                fetch(`combo_edit.php?get_tasks=1&level=${encodeURIComponent(level)}`)
