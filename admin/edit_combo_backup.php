            ...
            <div class="page-header">
                <div style="display: flex; align-items: center; gap: 16px;">
                    ...
                    <div>
                        ...
                        <p class="page-subtitle">Modify combo settings for ID: <?php echo $comboId; ?></p>
                    </div>
                </div>
            </div>
                    ...
                    <form method="POST">
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select id="level" name="level" required onchange="loadTasksByLevel()">
                                    ...
                                    <?php foreach ($levels as $level): ?>
                                        <option value="<?php echo htmlspecialchars($level); ?>" <?php echo $combo['level'] === $level ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <div class="user-select-container">
                                    <input type="text" id="user_select" name="user_select" placeholder="Search user by name or email..." autocomplete="off" value="<?php echo $assignedUser ? htmlspecialchars($assignedUser['fullname'] . ' - ' . $assignedUser['email']) : ''; ?>">
                                    <input type="hidden" id="user_id" name="user_id" value="<?php echo $combo['user_id'] ?: ''; ?>">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" id="amount" name="amount" step="0.01" min="0" value="<?php echo $combo['amount']; ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="status">Status</label>
                                <select id="status" name="status" required>
                                    <option value="active" <?php echo $combo['status'] === 'active' ? 'selected' : ''; ?>>Active</option>
                                    <option value="inactive" <?php echo $combo['status'] === 'inactive' ? 'selected' : ''; ?>>Inactive</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="message" name="message" required placeholder="Enter combo message for users" rows="3"><?php echo htmlspecialchars($combo['message']); ?></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label for="multiplier">Multiplier</label>
                            <input type="number" id="multiplier" name="multiplier" step="0.1" min="1" value="<?php echo $combo['multiplier']; ?>" required>
                        </div>
        ...
        function loadTasksByLevel() {
            ...
            fetch('get_tasks_by_level.php?level=' + encodeURIComponent(level))
                ...
                .then(data => {
                    ...
                    startSelect.value = '<?php echo $combo['start_task']; ?>';
                    endSelect.value = '<?php echo $combo['end_task']; ?>';
                })
                .catch(error => {
                    console.error('Error:', error);
                    startSelect.innerHTML = '<option value="">Error loading tasks</option>';
