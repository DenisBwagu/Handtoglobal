...
// Add missing columns to levels table before fetching/updating
$missingColumns = [
    "icon" => "VARCHAR(100) NOT NULL DEFAULT 'medal-bronze'",
    "task_type" => "VARCHAR(100) NOT NULL DEFAULT 'Name_items'",
    "reward" => "DECIMAL(10,2) NOT NULL DEFAULT 0.00",
    "tasks_count" => "INT NOT NULL DEFAULT 40",
...
foreach ($missingColumns as $column => $definition) {
    try {
        $conn->exec("ALTER TABLE levels ADD COLUMN `$column` $definition");
...
foreach ($missingColumns as $column => $definition) {
    try {
        $pdo->exec("ALTER TABLE levels ADD COLUMN `$column` $definition");
    } catch (PDOException $e) {
        // Column already exists, ignore
    }
}
...
// Safe migration for is_active column if it doesn't exist
try {
    $conn->exec("ALTER TABLE levels ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1");
} catch (PDOException $e) {
    // Column already exists, ignore
}
